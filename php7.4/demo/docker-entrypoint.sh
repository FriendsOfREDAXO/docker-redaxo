...
if [[ -n "$(ls -A)" ]]; then
    ...
    repeat=10
    echo >&2 "php redaxo/bin/console -q db:set-connection --host=$REDAXO_DB_HOST --database=$REDAXO_DB_NAME --login=$REDAXO_DB_LOGIN --password=$REDAXO_DB_PASSWORD"
